...
#. $HOME/.nvm/nvm.sh && type -P nvm || exec bash && . $HOME/.nvm/nvm.sh

type -P node || echo -e "node not in $PATH\nTrying to install..."; type -P apt-get && apt-get install node || type -P brew && brew install node || echo "install node"
